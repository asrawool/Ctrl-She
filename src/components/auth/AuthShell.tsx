...
export function AuthShell({
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg,#06132a 0%,#071A2E 55%,#0a2547 100%)",
      }}
    >
      {/* Left brand panel */}
      <div className="relative hidden lg:flex flex-col justify-between p-12 text-white overflow-hidden">
        ...
        <div className="relative">
          <h1 className="font-display text-5xl font-bold leading-tight">
            Unified{" "}
            <span className="text-gradient-cyan">Asset & Operations</span>{" "}
            Brain.
          </h1>
          <p className="mt-5 max-w-md text-white/70">
            Secure enterprise sign-in. Multi-factor authentication with
            biometric verification for regulated industrial environments.
          </p>
        </div>
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s} className="flex-1">
                  <div
                    className={`h-1.5 rounded-full transition-all ${state === "done" ? "bg-emerald" : state === "active" ? "bg-accent" : "bg-muted"}`}
                  />
                  <div
                    className={`mt-1.5 text-[10px] font-semibold uppercase tracking-wider ${state === "todo" ? "text-muted-foreground" : "text-foreground"}`}
                  >
                    {s}
                  </div>
                </div>
