...
import { ArrowRight, PlayCircle, ShieldCheck, Brain, Factory, FileText, Network, Wrench } from "lucide-react";
      ...
      <div className="relative mx-auto grid max-w-7xl gap-16 px-6 lg:grid-cols-2 lg:items-center">
        ...
        >
          ...
          <h1 className="mt-6 font-display text-5xl font-bold leading-[1.05] tracking-tight md:text-6xl lg:text-[68px]">
            ...
            <span className="text-gradient-cyan">Intelligence</span> for<br />
            Smarter Operations
          </h1>

          <p className="mt-6 max-w-xl text-lg text-white/70">
            Transform fragmented engineering knowledge into an intelligent, searchable, AI-powered
            operational brain. Empower maintenance, production, quality and safety teams to make
            faster, safer and better decisions.
          </p>

          <div className="mt-8 flex flex-wrap items-center gap-4">
            <a href="#demo" className="btn-hero inline-flex items-center gap-2 rounded-full px-7 py-3.5 text-sm font-semibold">
              Request Demo <ArrowRight className="h-4 w-4" />
            </a>
            <a href="#platform" className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-6 py-3.5 text-sm font-semibold text-white backdrop-blur hover:bg-white/10 transition">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 text-xs text-white/60">
            <div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-emerald" /> SOC 2 · ISO 27001</div>
            <div className="flex items-center gap-2"><Brain className="h-4 w-4 text-cyan" /> RAG + Knowledge Graph</div>
            <div className="flex items-center gap-2"><Factory className="h-4 w-4 text-cyan" /> Deployed at Fortune 500 plants</div>
          </div>
        </motion.div>
...
function OrbitVisual() {
  const nodes = [
    ...
    { icon: FileText, label: "Documents", cls: "top-1/2 right-0 -translate-y-1/2" },
    { icon: Wrench, label: "Assets", cls: "bottom-0 left-1/2 -translate-x-1/2" },
    ...
    <div className="relative h-full w-full">
      ...
      >
        <div className="relative grid h-40 w-40 place-items-center rounded-full bg-gradient-to-br from-cyan to-emerald shadow-[0_0_80px_rgba(0,194,255,0.55)]">
          ...
          <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-[#05122a] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-cyan border border-cyan/30">AI Core</div>
        </div>
      </motion.div>
      ...
      {nodes.map(({ icon: Icon, label, cls }, i) => (
        ...
        >
          <div className="glass-dark rounded-2xl px-4 py-3 flex items-center gap-2.5 text-white shadow-xl">
            ...
            <div>
              <div className="text-[11px] uppercase tracking-widest text-white/50">Node</div>
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 400 400" fill="none">
        ...
        {[[200,40,200,200],[360,200,200,200],[200,360,200,200],[40,200,200,200]].map(([x1,y1,x2,y2], i) => (
          <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="url(#glow)" strokeWidth="1.5" strokeDasharray="4 4">
