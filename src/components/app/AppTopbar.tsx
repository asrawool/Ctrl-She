...
import { Bell, Search, Moon, Sun, Globe, LogOut, User, Settings as SettingsIcon, ChevronDown } from "lucide-react";
...
const LANGS = [
  { c: "en", n: "English" }, { c: "hi", n: "हिन्दी" }, { c: "mr", n: "मराठी" },
  { c: "gu", n: "ગુજરાતી" }, { c: "ta", n: "தமிழ்" }, { c: "te", n: "తెలుగు" },
  { c: "bn", n: "বাংলা" }, { c: "kn", n: "ಕನ್ನಡ" }, { c: "pa", n: "ਪੰਜਾਬੀ" },
];

export function AppTopbar() {
  const { email, role, customRole, theme, toggleTheme, language, setLanguage, logout } = useAuth();
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border bg-background/95 backdrop-blur px-4 lg:px-6 h-16">
        ...
        {segs.map((s, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-1.5">
              ...
              <span className={`${last ? "font-semibold text-foreground" : "text-muted-foreground"} truncate`}>
                {LABELS[s] ?? s}
              </span>
            </div>
          );
      ...
      <div className="ml-auto hidden md:block relative">
        ...
        <input
          value={q} onChange={(e)=>{setQ(e.target.value);setSearchOpen(!!e.target.value)}}
        ...
        {searchOpen && (
          <div className="absolute right-0 top-12 w-96 rounded-xl border border-border bg-popover shadow-lg p-3">
            <div className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-2">Recent</div>
            {["Pump P-401 maintenance log","ISO 9001 audit checklist","Boiler B-12 incident report"].map((r)=>(
              <div key={r} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-muted cursor-pointer">
                <Search className="h-3.5 w-3.5 text-muted-foreground" />{r}
              </div>
            ))}
            <div className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mt-3 mb-2">Suggestions</div>
            <div className="text-xs text-muted-foreground italic">Type to search across documents, equipment, projects and knowledge articles.</div>
          </div>
        )}
      </div>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          {LANGS.map((l) => (
            <DropdownMenuItem key={l.c} onClick={() => setLanguage(l.c)}>{l.n}</DropdownMenuItem>
          ))}
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      <button onClick={toggleTheme}
        className="grid h-9 w-9 place-items-center rounded-lg border border-border hover:bg-muted transition" title="Toggle theme">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="flex items-center gap-2.5 rounded-lg pl-1 pr-2 py-1 hover:bg-muted transition">
          ...
          <div className="hidden lg:block text-left leading-tight">
            <div className="text-xs font-semibold truncate max-w-[140px]">{email}</div>
        ...
        <DropdownMenuContent align="end" className="w-56">
          <DropdownMenuLabel>
            ...
            <div className="text-[10px] font-normal text-muted-foreground truncate">{roleLabel}</div>
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => navigate({ to: "/app/profile" })}><User className="mr-2 h-4 w-4" /> Profile</DropdownMenuItem>
          <DropdownMenuItem onClick={() => navigate({ to: "/app/settings" })}><SettingsIcon className="mr-2 h-4 w-4" /> Settings</DropdownMenuItem>
          <DropdownMenuSeparator />
