          ...
          <aside className="rounded-2xl border border-border bg-card p-5 h-fit">
            {selected ? (
                ...
                <div className="mt-4 space-y-3 text-sm">
                  <div>
                    <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1.5">
                      ...
                      {
                        EDGES.filter(
                          (e) => e.a === selected.id || e.b === selected.id,
                        ).length
                      }
                      )
                    </div>
                    <div className="space-y-1">
                      {EDGES.filter(
                        ...
                        const other = NODES.find(
                          (n) => n.id === (e.a === selected.id ? e.b : e.a),
                        )!;
                        return (
                          <button
                            key={i}
                            onClick={() => setSelected(other)}
                            className="flex items-center gap-2 w-full rounded-lg px-2 py-1.5 text-left hover:bg-muted transition"
                          >
                            <span
                              className="h-2 w-2 rounded-full"
                              style={{ background: COLORS[other.type] }}
                            />
              ...
              <div className="text-center py-8 text-sm text-muted-foreground">
                <Filter className="mx-auto h-6 w-6 mb-2" />
...
function IconBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="grid h-9 w-9 place-items-center rounded-lg bg-white/10 hover:bg-white/20 text-white transition"
    >
      {children}
    </button>
  );
