...
function Copilot() {
  const [convs, setConvs] = useState<Conv[]>([
    { id: "c1", title: "New chat", ts: Date.now(), messages: [] },
  ]);
        ...
        c.id === activeId
          ? {
              ...
              messages: c.messages.map((m) =>
                m.id === aiMsg.id
                  ? { ...m, loading: false, text: res.answer }
                  : m,
              ),
            }
          : c,
    ...
    <div className="grid gap-4 lg:grid-cols-[280px_1fr] h-[calc(100vh-8rem)]">
      ...
      <aside className="hidden lg:flex flex-col rounded-2xl border border-border bg-card overflow-hidden">
        <div className="p-3 border-b border-border">
          <Button onClick={newChat} className="w-full btn-hero">
      ...
      <div className="flex flex-col rounded-2xl border border-border bg-card overflow-hidden">
            ...
            <AnimatePresence>
              {active.messages.map((m) => (
                <Bubble
                  key={m.id}
                  msg={m}
                  onRegen={() => send(active.messages.at(-2)?.text ?? "")}
                />
              ))}
            </AnimatePresence>
          )}
        ...
        <div className="border-t border-border p-3">
          ...
          <p className="mt-2 text-[10px] text-muted-foreground text-center italic">
            ...
            documentation before taking operational or safety-critical actions.
          </p>
        </div>
      </div>
    </div>
...
function IconBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="grid h-6 w-6 place-items-center rounded hover:bg-background transition"
    >
      <I className="h-3 w-3" />
    </button>
  );
