...
const frameworks = [
  ...
  { n: "PESO", s: 88, tone: "emerald" },
  { n: "ISO 9001", s: 96, tone: "emerald" },
  { n: "ISO 14001", s: 91, tone: "emerald" },
  { n: "ISO 45001", s: 83, tone: "warning" },
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={l} className="rounded-2xl border border-border bg-card p-4">
            <div
              className={`grid h-9 w-9 place-items-center rounded-lg ${tone === "emerald" ? "bg-emerald/10 text-emerald" : tone === "warning" ? "bg-orange-500/10 text-orange-500" : "bg-accent/10 text-accent"}`}
            >
            ...
            {frameworks.map((f) => (
              <div key={f.n} className="rounded-xl border border-border p-3">
                <div className="flex items-baseline justify-between mb-1">
                  ...
                  <span
                    className={`font-display text-lg font-bold ${f.tone === "warning" ? "text-orange-500" : "text-emerald"}`}
                  >
                    {f.s}%
                  </span>
                </div>
                <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                  <div
                    className={`h-full ${f.tone === "warning" ? "bg-orange-500" : "bg-emerald"}`}
                    style={{ width: `${f.s}%` }}
                  />
                </div>
              </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="space-y-2.5">
            {[
              {
                id: "NCR-2024-042",
                t: "Missing calibration record for gauge PG-201",
                sev: "High",
                s: "Open",
              },
              {
                id: "NCR-2024-041",
                t: "SOP deviation in reactor cleaning",
                sev: "Medium",
                s: "In Review",
              },
              {
                id: "NCR-2024-040",
                t: "Documented training gap — 3 operators",
                sev: "Low",
                s: "Closed",
              },
            ].map((n) => (
              <div key={n.id} className="rounded-xl border border-border p-3">
                <div className="flex items-center justify-between text-[11px] uppercase tracking-wider font-bold text-muted-foreground">
                  <span>{n.id}</span>
                  <span
                    className={`rounded-full px-2 py-0.5 ${n.sev === "High" ? "bg-destructive/10 text-destructive" : n.sev === "Medium" ? "bg-orange-500/10 text-orange-500" : "bg-emerald/10 text-emerald"}`}
                  >
                    {n.sev}
                  </span>
                </div>
