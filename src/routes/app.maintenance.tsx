    ...
    <>
        ...
        {equip.map((e) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className={`grid h-8 w-8 place-items-center rounded-lg ${e.status === "critical" ? "bg-destructive/10 text-destructive" : e.status === "warning" ? "bg-orange-500/10 text-orange-500" : "bg-emerald/10 text-emerald"}`}
              >
            ...
            <div className="mt-3">
              <div className="flex items-baseline justify-between text-xs mb-1">
                ...
                <span className="font-bold">{e.health}%</span>
              </div>
              <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                <div
                  className={`h-full ${e.status === "critical" ? "bg-destructive" : e.status === "warning" ? "bg-orange-500" : "bg-emerald"}`}
            ...
            <div className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <b className="text-foreground">{e.rul} days</b>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="relative pl-6 space-y-4 before:absolute before:left-2 before:top-1 before:bottom-1 before:w-0.5 before:bg-border">
            {[
              ...
              {
                d: "2d",
                t: "Predictive: HX-7 gasket replacement",
                tone: "warning",
              },
              {
                d: "5d",
                t: "Inspection: C-12 vibration analysis",
                tone: "cyan",
              },
              ...
              <div key={i} className="relative">
                <span
                  className={`absolute -left-6 top-1 grid h-4 w-4 place-items-center rounded-full ${e.tone === "warning" ? "bg-orange-500" : e.tone === "emerald" ? "bg-emerald" : "bg-accent"}`}
                />
          ...
          <div className="space-y-2">
            {[
              ...
              >
                ...
                <span
                  className={`font-display text-lg font-bold ${s.tone === "destructive" ? "text-destructive" : "text-emerald"}`}
                >
                  {s.stock}
                </span>
              </div>
      ...
      <div className="mt-6 rounded-2xl border border-border bg-card p-5">
        <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
          ...
          Analysis — IR-2024-118
        </h3>
        <div className="grid gap-4 md:grid-cols-3 text-sm">
          <RcaCol
            title="Symptom"
            items={[
              "Excessive vibration >4.2 mm/s",
              "Temperature rise 12°C above baseline",
              "Abnormal noise",
            ]}
          />
          <RcaCol
            ...
            items={[
              "Bearing starvation",
              "Lube interval too long (2000h)",
              "High-vibration operating regime",
            ]}
          />
          <RcaCol
            title="Action"
            items={[
              "Reduce lube interval to 1400h",
      ...
      <ul className="space-y-1.5 text-sm">
        {items.map((i) => (
          <li key={i} className="flex gap-2">
            <Wrench className="h-3 w-3 mt-1 text-accent shrink-0" />
            <span>{i}</span>
          </li>
        ))}
      </ul>
